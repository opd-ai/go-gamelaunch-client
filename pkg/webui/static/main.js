...
class ConfigurationManager {
  ...
  constructor() {
    ...
    this.defaultConfig = {
      ...
      input: {
        ...
        preventDefaultKeys: [
          ...
          'Tab', 'Space', 'Enter', 'Escape', 'Home', 'End',
          'PageUp', 'PageDown'
        ]
      },
      logging: {
    ...
    this.logger.info('constructor', 'Configuration manager initialized with defaults');
  }
  ...
  async loadConfiguration() {
    ...
    try {
      ...
      if (serverConfig) {
        ...
        this.logger.info('loadConfiguration', 'Configuration loaded from server');
      }
      ...
      if (localConfig) {
        ...
        this.configSource = this.configSource === 'server' ? 'server+local' : 'local';
        this.logger.info('loadConfiguration', 'Local configuration preferences applied');
      }
      ...
      if (urlConfig) {
        ...
        this.logger.info('loadConfiguration', 'URL parameter overrides applied');
      }
      ...
      this.logger.error('loadConfiguration', 'Configuration loading failed', error);
  ...
  async _loadServerConfig() {
    try {
      ...
      if (response.ok) {
        ...
        this.logger.debug('_loadServerConfig', 'Server configuration retrieved');
        return serverConfig;
      }
    } catch (error) {
      this.logger.debug('_loadServerConfig', 'Server configuration not available', error);
    }
    return null;
  }
  ...
  _loadLocalConfig() {
    try {
      ...
      if (stored) {
        ...
        this.logger.debug('_loadLocalConfig', 'Local configuration loaded from storage');
        return localConfig;
      }
    } catch (error) {
      this.logger.warn('_loadLocalConfig', 'Failed to load local configuration', error);
    }
    return null;
  }
  ...
  _loadUrlConfig() {
    ...
    for (const [param, configPath] of Object.entries(paramMappings)) {
      ...
      if (value !== null) {
        this._setNestedProperty(urlConfig, configPath, this._parseUrlValue(value));
        hasParams = true;
      }
    }
  ...
  _validateConfiguration() {
    ...
    if (!Object.values(RenderMode).includes(config.display.renderMode)) {
      this.logger.warn('_validateConfiguration', `Invalid render mode: ${config.display.renderMode}, using default`);
      config.display.renderMode = RenderMode.HYBRID;
    }
    
    // Validate font size
    if (config.display.fontSize < 8 || config.display.fontSize > 32) {
      this.logger.warn('_validateConfiguration', `Invalid font size: ${config.display.fontSize}, using default`);
      config.display.fontSize = 14;
    }
    
    // Validate poll intervals
    if (config.advanced.pollInterval < 50) {
      this.logger.warn('_validateConfiguration', 'Poll interval too low, adjusting to minimum');
    ...
    if (!validLevels.includes(config.logging.level)) {
      this.logger.warn('_validateConfiguration', `Invalid log level: ${config.logging.level}, using INFO`);
      config.logging.level = 'INFO';
    }
    
    this.logger.debug('_validateConfiguration', 'Configuration validation complete');
  }
  ...
  saveUserPreferences(preferences) {
    try {
      ...
      this.logger.info('saveUserPreferences', 'User preferences saved to local storage');
      
    } catch (error) {
      this.logger.error('saveUserPreferences', 'Failed to save user preferences', error);
    }
  }
...
class ApplicationManager {
  ...
  _setState(newState, reason = null) {
    ...
    this.logger.info('_setState', `Application state: ${previousState} -> ${newState}`, { reason });
  ...
  _emitEvent(eventType, eventData) {
    ...
    for (const listener of listeners) {
      try {
        ...
        this.logger.error('_emitEvent', `Error in ${eventType} event listener`, error);
      }
    }
  }
  ...
  addEventListener(eventType, listener) {
    ...
    this.logger.debug('addEventListener', `Event listener registered for: ${eventType}`);
  }
  ...
  removeEventListener(eventType, listener) {
    ...
    if (index !== -1) {
      ...
      this.logger.debug('removeEventListener', `Event listener removed for: ${eventType}`);
    }
  }
  ...
  async initialize() {
    ...
    try {
      ...
      this.logger.error('initialize', 'Application initialization failed', error);
  ...
  async _initializeDisplay() {
    ...
    try {
      ...
      this.logger.info('_initializeDisplay', 'Game display initialized successfully');
      
    } catch (error) {
      this.logger.error('_initializeDisplay', 'Display initialization failed', error);
      throw error;
    }
  }
  ...
  _setupErrorHandling() {
    ...
    window.addEventListener('unhandledrejection', (event) => {
      this.logger.error('_setupErrorHandling', 'Unhandled promise rejection', event.reason);
      this._recordError(event.reason, 'unhandled_promise');
    });
    
    this.logger.debug('_setupErrorHandling', 'Global error handlers configured');
  }
  ...
  _setupWindowHandlers() {
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      this.logger.debug('_setupWindowHandlers', `Page visibility: ${isVisible ? 'visible' : 'hidden'}`);
    ...
    window.addEventListener('beforeunload', (event) => {
      this.logger.info('_setupWindowHandlers', 'Page unloading, performing cleanup');
      this._shutdown();
    });
    ...
    this.logger.debug('_setupWindowHandlers', 'Window event handlers configured');
  }
  ...
  updatePreferences(preferences) {
    this.logger.info('updatePreferences', 'Updating user preferences', preferences);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (autoContainer) {
    ...
    client.onStateChange = (data) => {
      logger.info('AutoInit', `Client state: ${data.newState}`, { reason: data.reason });
    };
...
console.log('[WebGameClient] dgamelaunch web client main module loaded successfully');