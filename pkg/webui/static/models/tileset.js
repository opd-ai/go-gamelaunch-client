...
class TileSprite {
  ...
  isValidForGrid(gridWidth, gridHeight) {
    return this.x >= 0 && this.x < gridWidth && 
           this.y >= 0 && this.y < gridHeight;
  }
...
class SpriteCache {
  ...
  constructor(maxSize = 1000) {
    ...
    this.logger.info('constructor', `Sprite cache initialized with max size: ${maxSize}`);
  }
...
class Tileset {
  ...
  constructor(data = {}) {
    ...
    this.logger.info('constructor', `Tileset created: ${this.name}, tile size: ${this.tile_width}x${this.tile_height}`);
  }
  ...
  _calculateGridDimensions() {
    if (!this.imageLoaded || !this.imageElement) {
      this.logger.warn('_calculateGridDimensions', 'Cannot calculate dimensions: image not loaded');
      return;
    }
    ...
    this.logger.info('_calculateGridDimensions', 
      `Grid calculated: ${this.tiles_per_row}x${calculatedTotalRows} (${this.total_tiles} tiles)`);
  }
  ...
  async loadImage(imageUrl = null) {
    ...
    if (this.loadPromise) {
      this.logger.debug('loadImage', 'Image load already in progress, returning existing promise');
    ...
    this.loadPromise = new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        this.logger.info('loadImage', `Image loaded successfully in ${loadTime}ms`, {
          dimensions: `${img.naturalWidth}x${img.naturalHeight}`,
          gridSize: `${this.tiles_per_row}x${Math.floor(this.imageHeight / this.tile_height)}`
        });
      ...
      img.onerror = () => {
        ...
        this.logger.error('loadImage', `Image load failed after ${loadTime}ms`, error);
  ...
  isValidCoordinate(x, y) {
    ...
    const maxRows = this.imageHeight ? Math.floor(this.imageHeight / this.tile_height) : Infinity;
  ...
  setDefaultSprite(x, y) {
    ...
    if (sprite) {
      this.defaultSprite = sprite;
      this.logger.info('setDefaultSprite', `Default sprite set to (${x}, ${y})`);
    } else {
      this.logger.warn('setDefaultSprite', `Cannot set default sprite: invalid coordinates (${x}, ${y})`);
    }
  }
  ...
  getStats() {
    const stats = {
      ...
      imageDimensions: this.imageLoaded ? `${this.imageWidth}x${this.imageHeight}` : 'not loaded',
      tileDimensions: `${this.tile_width}x${this.tile_height}`,
      gridSize: this.tiles_per_row ? `${this.tiles_per_row}x${Math.floor(this.imageHeight / this.tile_height)}` : 'unknown',
  ...
  toJSON(includeSprites = false) {
    ...
    if (includeSprites) {
      result.sprites = Array.from(this.sprites.values()).map(sprite => sprite.toJSON());
      result.defaultSprite = this.defaultSprite ? this.defaultSprite.toJSON() : null;
    }
...
class TilesetLoader {
  ...
  async loadTileset(config, preloadImage = true) {
    ...
    if (this.loadingPromises.has(name)) {
      this.logger.debug('loadTileset', `Tileset ${name} already loading, returning existing promise`);
  ...
  unloadTileset(name) {
    ...
    this.logger.warn('unloadTileset', `Tileset not found for unloading: ${name}`);
    return false;
  }
